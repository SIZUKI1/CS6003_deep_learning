...
YOLOv8 是一种先进的 Anchor-free 单阶段目标检测模型。其主干网络采用了跨阶段局部网络（CSPNet）的设计思想，提取丰富的多尺度特征；颈部采用 PANet 结构实现自顶向下与自底向上的特征融合。YOLOv8 的重大改进在于引入了解耦头（Decoupled Head），将分类与回归任务分离。
在损失函数设计上，YOLOv8 的分类损失采用了二元交叉熵损失（BCE Loss），而边界框回归采用了 Distribution Focal Loss (DFL) 与 CIoU Loss 的加权组合。其中 CIoU Loss 考虑了真实框与预测框的重叠面积、中心点距离和长宽比，有效提升了复杂场景下的定位精度。

\subsubsection{ByteTrack 多目标跟踪原理}
由于 YOLOv8 仅对单帧进行独立检测，本实验引入 ByteTrack 作为多目标跟踪引擎。ByteTrack 通过卡尔曼滤波（Kalman Filter）对目标的运动状态进行线性预测，并利用匈牙利算法完成预测框与当前帧检测框的匹配。
其核心创新在于\textbf{两阶段关联策略}：第一阶段仅将高置信度检测框与已有轨迹匹配；第二阶段将未匹配的轨迹与低置信度检测框再次匹配。当车辆被短暂遮挡导致置信度骤降时，该机制能够“捡回”低分目标，有效维持 Tracking ID 的稳定性。
...
虽然模型达到了 0.521 的 mAP@0.5，但由于数据集中包含了多达 21 个类别，且部分类别特征极为相似，这导致了细粒度分类的困难。在真实的测试视频中，我们观察到一个典型的错误情况：即使目标被成功连续跟踪（保持同一个 Track ID），其预测类别也会出现突然变换的“闪烁”现象。例如在分析截图（图 2）中，一辆蓝色客车（ID:147）在连续帧中由 \textit{pickup} 突然变为 \textit{van}。这主要是因为本实验直接提取当前帧 YOLO 的独立检测类别作为最终输出，缺乏对时序轨迹上类别一致性的投票或平滑约束；加上远端车辆像素较小，外观特征极易受角度变化影响，从而导致了同一轨迹类别检测的剧烈波动。

\subsubsection{遮挡导致的 ID 跳变分析}
在测试视频中，系统自动检测到了多达 51 次 ID 跳变。结合实际画面观察（如图 2），这主要是因为车辆在汇向路口远端时，透视效应使得远端的车辆视觉分布变得极其密集；同时，车辆之间频繁的相互变道穿插，导致了大量的动态互相遮挡。以图 2 中的棕色三轮车为例，深刻揭示了跳变发生的过程：
\begin{enumerate}
